	...
	$(document)
			.ready(
					function() {
						$('#updateBtn').hide();
						$.ajax({
									url : './CategoryServlet?action=getAll',
									type : 'GET',
									success : function(data) {
										for (var i = 0; i < data.length; i++) {
											$('#authorTable').append('<tr id='+data.id+'><td data-target=name>'+ data.name+'</td><td> <a class="edit" data-id='+data.id+' title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a><a class="delete" data-id='+data.id+' title="Delete" data-toggle="tooltip"><i class="material-icons">&#xE872;</i></a></td>');
										}
									}
								});
					...
					$('#addBtn').on('click', function(){
						...
						$.ajax({
							url:'./CategoryServlet?action=create&&name='+name,
							type: "POST",
							success:function(data){
						...
						$(document).on("click",".edit",function() {
							...
							$('#updateBtn').show();
									...
									$('#updateBtn').on('click', function(){
										...
										$.ajax({
											url:'./CategoryServlet?action=update&&id='+id+'&&name='+name,
											type: "POST",
											success:function(data){
												$('#exampleModal').modal('hide');
												$('#author_form')[0].reset();
						...
						$(document).on("click", ".delete", function() {
							...
							if (confirm('Are you sure you want to delete this record?')) {
								$.ajax({
									url: './CategoryServlet?action=delete&&id='+id,
									type: 'Post',
